    ...
    <main>
      <section className="max-w-5xl mx-auto px-5 pt-10 pb-8 grid md:grid-cols-[1.1fr_.9fr] gap-8 items-center">
        <div className="animate-fade-up"><p className="section-label mb-3">● Live discoveries across Bengaluru</p>
          <h1 className="text-4xl md:text-6xl">Find rentals the <span className="text-accent">internet missed.</span></h1>
          <p className="text-slate max-w-md mt-5 mb-6 leading-relaxed text-lg">Real homes spotted on real streets — verified, fresh, and shared with you before they disappear.</p>
          <div className="flex gap-3 flex-wrap"><Link href="/discover" className="btn btn-primary press">Explore rentals <span>→</span></Link><Link href="/scout" className="btn press">I spotted a board</Link></div>
          <p className="text-[13px] text-slate mt-5">No brokers. No endless walking. From ₹29 to connect.</p></div>
        <div className="sticker p-6 relative overflow-hidden animate-fade-up" style={{ animationDelay: '120ms' }}><p className="section-label text-accent">{latest ? 'Just spotted' : 'Your local network'}</p><h2 className="text-2xl mt-2">{latest ? `${latest.bhk} BHK in ${latest.landmark}` : 'The next home is around the corner.'}</h2>{latest && <p className="font-mono font-bold text-xl mt-2">₹{latest.rent.toLocaleString('en-IN')} <span className="text-xs font-normal text-slate">/ month</span></p>}<div className="mt-5 bg-canvas border border-line rounded-xl p-3 text-sm">{latest ? <><div className="flex justify-between text-slate"><span>Approx. {latest.landmark}</span><span>Trust {latest.trustScore}</span></div><div className="flex items-center mt-3"><span className={`badge ${latest.status === 'verified' ? 'badge-green' : 'badge-yellow'}`}>{latest.status === 'verified' ? <><CheckIcon className="w-3 h-3" /> Owner verified</> : 'Community spotted'}</span><Link href={`/listing/${latest.id}`} className="ml-auto text-xs font-semibold text-accent">View →</Link></div></> : <p className="leading-relaxed text-slate">Scouts spot fresh TO-LET boards and renters connect directly — without walking every lane.</p>}</div></div>
      </section>
